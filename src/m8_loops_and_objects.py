...
def draw_circles1():
    ...
    for k in range(21):
        circle_k = rg.Circle(center_point, k*10)
        circle_k.attach_to(window)
    window.render()
...
def draw_circles2():
    ...
    for k in range(18):
        number = number + 20
        center_point = rg.Point(number, 100)
        circle_k = rg.Circle(center_point, 10)
        circle_k.fill_color = 'blue'
        circle_k.attach_to(window)
    window.render()
    window.close_on_mouse_click()
